...
class UtilityClass: NSObject {
    ...
    func requestGetFor(reqUrl:String,viewLoader:UIView,viewContLoader:UIViewController, completionHandler: @escaping (_ data : Data) -> Void) {
        let task = URLSession.shared.dataTask(with: URL(string: reqUrl)!) { (data2, response, error2) in
            
            guard let dataResponse = data2,
                error2 == nil else {
                    print(error2?.localizedDescription ?? "Response Error")
                    completionHandler(data2 ?? Data())
